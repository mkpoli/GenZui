(async () => {
 ...
 for(const button of $$('.glyph-card .swap')){
  ...
  button.click();check(id+' restores current',!sample.classList.contains('is-previous')&&uses(sample,'GenZui')&&sample.querySelector('.version-label').textContent==='0.116');
 }
 ...
 check('current download',$('footer a[href$=".zip"]').href.includes('GenZuiSerif-0.116.zip'));
