(async () => {
 ...
 change('#collection','revised');check('only hooked WU revised',visible().length===1);
 ...
 for(const cp of ['1b123','1b124','1b125','1b126','2a708','2cf00','2ceff','1b128']){
  const card=$('#u'+cp);
 ...
 check('curved WU shown once',getComputedStyle($('#u1b11f .before')).display==='none'&&getComputedStyle($('#u1b11f .swap')).display==='none');
 ...
 check('hooked WU uses matching features in both versions',feature($('#u1b11f .current .reading'))==='"ss01"'&&feature($('#u1b11f .before .reading'))==='"ss01"');
 ...
 $('#overlay').click();
 check('hooked overlay matches selected form',getComputedStyle($('#u1b11f .alternate-overlay')).display!=='none'&&getComputedStyle($('#u1b11f .default-overlay')).display==='none');
 $('#overlay').click();
 ...
 for(const button of $$('.glyph-card .swap')){
  ...
  check(id+' swaps drawing and both contexts',button.getAttribute('aria-pressed')==='true'&&sample.querySelector('.version-label').textContent==='0.110'&&uses(sample,'Previous')&&getComputedStyle(sample.querySelector('.previous-outline')).display!=='none');
  ...
  button.click();check(id+' restores current',!sample.classList.contains('is-previous')&&uses(sample,'GenZui')&&sample.querySelector('.version-label').textContent==='0.111');
 }
 $('#u1b11f .swap').click();change('#wu-style','curved');
 ...
 check('current download',$('footer a[href$=".zip"]').href.includes('0.111'));
