...
def build():
    ...
    samples = [
        ...
        ('Small Kana Extension', ''.join(chr(cp) for cp in [0x1B132, *range(0x1B150, 0x1B153), 0x1B155, *range(0x1B164, 0x1B169)])),
    ...
    *{box-sizing:border-box}body{margin:0;background:#f6f4ee;color:#252e28;font:16px/1.6 system-ui,sans-serif}main{max-width:1180px;padding:36px;margin:auto}h1{font-size:30px;line-height:1.25}h2{font-size:21px;margin-top:32px}a{color:#176953}button{background:#215c49;color:white;border:0;border-radius:5px;padding:11px 18px;font:inherit;cursor:pointer}button:disabled{background:#62776c}textarea{width:100%;height:260px;font:13px/1.5 Consolas,monospace;padding:15px;background:white;border:1px solid #bbb}table{border-collapse:collapse;width:100%;background:white}th,td{border:1px solid #cbd0c6;padding:12px;text-align:center}th{font-size:14px}td{font-size:36px;line-height:1.6}.auto{font-family:"Noto Sans JP",sans-serif}.reference{font-family:"GenSeki Hentaigana Gothic"}.scroll{overflow-x:auto}code{font-size:.92em}pre{white-space:pre-wrap;overflow-wrap:anywhere}.note{background:#e8eee6;padding:14px;border-left:3px solid #527a5f}#copy-status{margin-left:10px}details{margin-top:20px}
    ...
    <p><a href="historical-kana-fallback.user.js">Install or update the userscript · version 0.2.0</a></p>
    <p>This update fixes font ordering on Japanese Wikipedia's Small Kana Extension chart. Historical kana use GenSeki; ordinary text keeps the page's fonts.</p>
    <ol><li>Open the install link above and choose <strong>Update</strong> or <strong>Install</strong> in Tampermonkey.</li><li>If the link shows plain source code, open the existing <strong>Historical kana font fallback</strong> script in Tampermonkey's dashboard, replace its contents with the code below, and press <strong>Ctrl+S</strong>. For a first installation, choose <strong>Create a new script</strong>.</li><li>If Tampermonkey requests permission to run scripts, enable <strong>Allow User Scripts</strong> on its extension details page. See <a href="https://www.tampermonkey.net/faq.php?locale=en&amp;q=Q209">the official instructions</a>.</li><li>Reload the affected web page.</li></ol>
    ...
    <p>The left column requests Noto Sans JP and sans-serif; the right column requests GenSeki. The button below enables the helper for this preview. This preview does not install the userscript or check whether it is enabled on other websites.</p>
